...
def mutable_dataset_dict(title, name, month, resource=[], **kwargs):
    return {
        "title": title,
        "name": name,
...
def generate_dataset_dict():
    ...
    for file in fd_files:
        dataset_dict["datasets"].append(mutable_dataset_dict(
            title="Family Medicine Reports" + " for " + MONTHS[int(file["month"])] + " " + file["year"],
            name="family-medicine-reports" + "-" + (file["month"]) + "-" + (file["year"]),
            month=file["year"] + "-"+ file["month"],
            year=file["year"],
            notes="WHO ROMANIA - Family Medicine Reports for " + MONTHS[int(file["month"])] + " " + file["year"],
            resource=[read_resource_file(file) for file in file["files"]]
        ))
